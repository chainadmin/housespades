...
- **Bot transparency (Guideline 5.6):** All computer players show a "BOT" badge in PlayerZone (driven by `isBot`). Matchmaking screen discloses ~10s bot seat-filling. Disconnected/idle players keep their name when a bot takes over their seat (badge appears; no human-like rename). Idle turns auto-play once; two consecutive missed turns hand the seat to a bot. `/api/purchase/verify` is disabled (501) until real receipt validation exists. Reviewer docs: `mobile/APP_REVIEW_NOTES.md`, `mobile/RESOLUTION_CENTER_REPLY.md`.
