...
- **Sound Effects (mobile):** 7 synthesized royalty-free WAVs in `mobile/assets/sounds/` played via `expo-audio` through `mobile/lib/sound.ts` (initSounds/playSound; mute persisted in SecureStore; queues sounds fired before async init completes). Wired in `game.tsx`: deal on bidding entry, bid, card-play (via trick-length effect so bot/opponent plays sound too), trick-win, your-turn, win/lose at game over. Toggle in Settings → Preferences.
- **Tutorial (mobile):** 5-step skippable modal (`mobile/components/Tutorial.tsx`) auto-shown on first launch (SecureStore flag `house_spades_tutorial_seen`), replayable from Settings → "How to Play Tutorial".
- **Animations (mobile):** Reanimated — trick cards slide in from the seat that played them (spring) with winner zoom/glow, hand cards stagger in on deal, game-over modal zooms in.
- **iOS is phone-only:** `supportsTablet: false` in app.json (deliberate — avoids iPad review surface).
- **Native iOS APIs (App Store 4.2 compliance):** App uses Core Haptics (`expo-haptics`) on bid/play/trick-win/game-over, native share sheet (`Share` API) on multiplayer game-over, and a felt-textured `ImageBackground` on the game table. Plus secure Keychain storage (`expo-secure-store`), AdMob with App Tracking Transparency, native WebSockets, and native gesture/animation libraries. Reviewer notes live in `mobile/APP_REVIEW_NOTES.md`. Local notifications via `expo-notifications` were removed in iOS build 37 because the package's `aps-environment` entitlement was blocking signing against the existing provisioning profile (we don't need push); the package is uninstalled — do not re-add it without first regenerating the provisioning profile with Push Notifications capability.
- **Bot transparency (Guideline 5.6):** All computer players show a "BOT" badge in PlayerZone (driven by `isBot`). Matchmaking screen discloses ~10s bot seat-filling. Disconnected/idle players keep their name when a bot takes over their seat (badge appears; no human-like rename). Idle turns auto-play once; two consecutive missed turns hand the seat to a bot. `/api/purchase/verify` validates purchases server-to-server via RevenueCat before granting `remove_ads`. Reviewer docs: `mobile/APP_REVIEW_NOTES.md`, `mobile/RESOLUTION_CENTER_REPLY.md`.
- **Mobile bidding layout:** BiddingPanel renders as a compact full-width bar between the game table and the player's hand (never inside the center table area) so the flexible table absorbs its height — keeps everything on-screen on small iPhones. Center area always shows TrickArea.
...
- **Leaderboard:** Public endpoint `GET /api/leaderboard?limit=N` returns top players (filtered to `gamesPlayed > 0`, ordered by rating desc then wins desc, capped at 100). Web page at `/leaderboard` (route in `client/src/App.tsx`, page in `client/src/pages/Leaderboard.tsx`, header link via Award icon on Home). Mobile screen at `/leaderboard` (registered in `mobile/app/_layout.tsx`, page in `mobile/app/leaderboard.tsx`, header trophy icon on home). Highlights the current user's row, shows medals for top 3, pull-to-refresh on mobile.
...
- **RevenueCat (in-app purchases):** "Remove Ads" is a one-time IAP. Mobile purchases via `react-native-purchases` (`mobile/hooks/usePurchases.ts`) with the RevenueCat app user ID set to the server user id; the server (`server/purchases.ts`) verifies the `remove_ads` entitlement with RevenueCat's REST API before setting `users.remove_ads`. Public SDK keys must be filled into `mobile/app.json` extra (`revenueCatAppleApiKey` / `revenueCatGoogleApiKey`) or `EXPO_PUBLIC_REVENUECAT_*` env vars at build time — the stored `IOS_RevenueCat_API_KEY` / `Google_RevenueCat_API_KEY` secrets are secret (`sk_`) keys used server-side only and must never ship in the app.